...
function setClass(node){
	if(node.children.length>0&&node.id!="container"&&node.children[0].className!="icon"){
		var newSpan=document.createElement("span");
		newSpan.className="icon";
		newSpan.innerHTML="+";
		node.insertBefore(newSpan,node.firstChild);
	}
	...
	for(var j=0;j<node.children.length;j++){
		if(node.children[j].children&&node.children[j].nodeName=="DIV"){
...
function animation(arr,stop) {
	...
	function changeBg(){
		renderDiv();
		if(i<arr.length){
			treeList[arr[i]].parentNode.firstChild.innerHTML="-";
			renderChildDisplay(treeList[arr[i]].parentNode,"block");
			treeList[arr[i]].className+=" choosen";
			var targetValue=treeList[arr[i]].firstChild.nextSibling||treeList[arr[i]].firstChild;
			if(targetValue.nodeValue.trim()==stop){
				alert("已找到您要搜寻的内容");
				clearInterval(clear);
				return;
			}
...
function divClick(){
	...
	if(target.className=="icon"){
		...
		case "+":
			target.innerHTML="-";
			renderChildDisplay(parentNode,"block");
			break;
		case "-":
			target.innerHTML="+";
			renderChildDisplay(parentNode,"none");
			break;
		}
	}else{
		renderDiv();
...
//更新节点显示方式
function renderChildDisplay(node,style){
	var len=node.children.length;
	for(var i=0;i<len;i++){
		if(node.children[i].nodeName!="SPAN"){
			node.children[i].style.display=style;
		}
	}
}
//初始化函数
